...
if(isset($_POST['Ajouter'])){
  ...
  $target_dir = "uploads/".$category."/".$selectedValue."/";
   ...
   echo '<script>alert("Veuillez remplir le champ nouveauté!")</script>';
    ...
    echo $result = mysqli_query($dbconn,$sql);
    header('Location: physFLoggedIn.php');
   }
   else{
...
if(isset($_POST['Supprimer'])){
 ...
 $sql="DELETE FROM lessons WHERE fileLocation='".$location."' AND category='physFYear'";
  echo $result = mysqli_query($dbconn,$sql);
 header('Location: physFLoggedIn.php');
}
?>
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav">
                ...
                <div class="dropdown-menu active" aria-labelledby="dropdownMenuButton">
                  ...
                  <a class="dropdown-item active" href="firstyearFilesLoggedIn.php">Première</a>
    ...
    <li>Cours(.ZIP/.RAR):
      <ul>
        <?php 
         ...
         $result = mysqli_query($dbconn,"SELECT fileLocation FROM lessons WHERE format='Cours' AND category='physFYear'");
    ...
    <li>Exos:
      <ul>
        <?php 
         ...
         $result = mysqli_query($dbconn,"SELECT fileLocation FROM lessons WHERE format='Exo' AND category='physFYear'");
         while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
           echo '<li><form method=\'post\' action=""><a href="'.$row[0].'" download>'.basename($row[0]).'</a><button class="btn" type="submit" name="Supprimer" value="'.$row[0].'"><i class="fa fa-trash"></i></button></form></li>' ;
          }
          ?>
    ...
    <li>Corrigés:
      <ul>
        <?php 
         ...
         $result = mysqli_query($dbconn,"SELECT fileLocation FROM lessons WHERE format='Corrigé' AND category='physFYear'");
         while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
           echo '<li><form method=\'post\' action=""><a href="'.$row[0].'" download>'.basename($row[0]).'</a><button class="btn" type="submit" name="Supprimer" value="'.$row[0].'"><i class="fa fa-trash"></i></button></form></li>' ;
          }
          ?>
    ...
    <li>Enregistrement des cours(Vidéo):
       <ul>
        <?php 
         ...
         $result = mysqli_query($dbconn,"SELECT fileLocation FROM lessons WHERE format='Enregistrement des c' AND category='physFYear'");
         while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
           echo '<li><form method=\'post\' action=""><a href="'.$row[0].'" download>'.basename($row[0]).'</a><button class="btn" type="submit" name="Supprimer" value="'.$row[0].'"><i class="fa fa-trash"></i></button></form></li>' ;
          }
          ?>
